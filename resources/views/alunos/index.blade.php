...
@section('breadcrumb')
    @include('layout.dashboard_breadcrumb', ['title' => ['name' => 'Alunos', 'path' => ['Alunos']]])
@endsection
        ...
        <div class="row">
            <div class="col">
                <div class="white-box">
                    <div class="row mb-2">
                        <div class="col">
                            <h4 class="box-title">Lista de Alunos</h4>
                        </div>
                        <div class="col text-right">
                            <a href="{{ url('/alunos/create') }}" type="button" class="btn btn-primary">Novo Aluno</a>
                        </div>
                    </div>
                    <table class="table table-hover table-sm" id="dt-alunos">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Nome</th>
...
@section('javascript')
    ...
    <script type="text/javascript">
        let url_base = "{{ url('/alunos')}}";

        $(document).ready(function() {
            jQuery.fn.dataTable.ext.errMode = 'none';
            table = $('#dt-alunos').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route ('alunos') }}",
                ...
                "initComplete": function(settings, json) {
                    table.buttons().container().appendTo('#dt-alunos_wrapper .col-md-4:eq(0)');
